...
namespace DX
{
    ...
    public:
        ...
        {
            if (!QueryPerformanceCounter(&m_qpcLastTime)) throw std::exception("QueryPerformanceCounter");
        ...
        void Tick(const TUpdate& update)
        {
            ...
            if (!QueryPerformanceCounter(&currentTime)) throw std::exception("QueryPerformanceCounter");
            ...
            {
                m_elapsedTicks = timeDelta; m_totalTicks += timeDelta;
                m_leftOverTicks = 0; m_frameCount++; update();
            }
            if (m_frameCount != lastFrameCount) m_framesThisSecond++;
            if (m_qpcSecondCounter >= static_cast<uint64_t>(m_qpcFrequency.QuadPart))
            {
                m_framesPerSecond = m_framesThisSecond; m_framesThisSecond = 0;
    ...
    private:
        ...
        uint64_t m_qpcMaxDelta, m_elapsedTicks, m_totalTicks, m_leftOverTicks;
        uint32_t m_frameCount, m_framesPerSecond, m_framesThisSecond;
