...
void GameScene::DrawWorldArena()
{
    ...
    floorPalette.topColor = Color(0.09f, 0.16f + floorPulse * 0.08f, 0.20f + floorPulse * 0.06f, 0.96f);
    if (m_floorStyle)
    {
    ...
    const Color rimColor = StageTint(Color(
        floorPalette.accentColor.R(),
        floorPalette.accentColor.G(),
        floorPalette.accentColor.B(),
        0.55f + floorPulse * 0.25f));
    ...
    std::vector<FloorDrawCommand> floorDetailCommands;
    ...
    {
        const FloorDrawCommand& command = floorDetailCommands[commandIndex];
        m_obstacleMesh->Draw(command.world, m_view, m_proj, StageTint(command.color));
    }

    const auto DrawTheme2Strips = [&]()
    {
        for (int lane = 0; lane < 3; ++lane)
        {
            const float z = -14.0f + static_cast<float>(lane) * 14.0f;
            const Matrix stripA =
    ...
    const auto DrawTheme3Arcs = [&]()
    {
        (void)stagePulse;
    };

    const auto DrawTheme2Walls = [&]()
    {
    };

    const auto DrawTheme3CenterDais = [&]()
    {
