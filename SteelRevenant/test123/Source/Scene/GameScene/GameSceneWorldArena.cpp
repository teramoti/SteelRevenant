//------------------------//------------------------
// Contents(蜃ｦ逅・・螳ｹ) 繧｢繝ｪ繝ｼ繝翫・蠎翫・螢√・繧ｹ繝昴・繝ｳ繝昴う繝ｳ繝医ｒ螳溯｣・☆繧九・// Bug#4菫ｮ豁｣: 荳ｭ邯吝慍轤ｹ繝ｻ繝上じ繝ｼ繝峨・繝薙・繧ｳ繝ｳ縺ｸ縺ｮ蜿ら・繧貞ｮ悟・髯､蜴ｻ縲・//------------------------//------------------------
...
{
    m_floorHalfExt = 24.0f;
    const float W = 24.0f, H = 3.5f, T = 1.0f;
    ...
    for (int s=-1;s<=1;s+=2) for (int t=-1;t<=1;t+=2) {
        const float px=static_cast<float>(s)*16.0f, pz=static_cast<float>(t)*16.0f;
    ...
    const float sp = 18.0f;
    m_spawnPoints = {
        { sp,0.8f,0},{-sp,0.8f,0},{0,0.8f, sp},{0,0.8f,-sp},
        { sp,0.8f, sp},{-sp,0.8f, sp},{ sp,0.8f,-sp},{-sp,0.8f,-sp},
        { 10.0f,0.8f, sp},{-10.0f,0.8f, sp},{ 10.0f,0.8f,-sp},{-10.0f,0.8f,-sp},
        { sp,0.8f, 10.0f},{ sp,0.8f,-10.0f},{-sp,0.8f, 10.0f},{-sp,0.8f,-10.0f}
    };
}
...
void GameSceneWorldArena::BuildStage2()
{
    ...
    for (int side=-1;side<=1;side+=2) for (int row=-1;row<=1;++row) {
        const float bx=static_cast<float>(side)*12.0f, bz=static_cast<float>(row)*8.0f;
    ...
    const float sp=19.0f;
    m_spawnPoints = {
        { sp,0.8f,0},{-sp,0.8f,0},{ sp,0.8f, 8},{ sp,0.8f,-8},
        {-sp,0.8f, 8},{-sp,0.8f,-8},{0,0.8f, sp},{0,0.8f,-sp},
        { sp,0.8f,12},{ sp,0.8f,-12},{-sp,0.8f,12},{-sp,0.8f,-12},
        { 8.0f,0.8f, sp},{-8.0f,0.8f, sp},{ 8.0f,0.8f,-sp},{-8.0f,0.8f,-sp}
    };
}
...
void GameSceneWorldArena::BuildStage3()
{
    ...
    addWall(0,W,W*2,T); addWall(0,-W,W*2,T); addWall(W,0,T,W*2); addWall(-W,0,T,W*2);
    for (int s=-1;s<=1;s+=2) for (int t=-1;t<=1;t+=2) {
        const float ox=static_cast<float>(s)*14.0f, oz=static_cast<float>(t)*14.0f;
    ...
    m_spawnPoints = {
        { sp,0.8f, sp},{-sp,0.8f, sp},{ sp,0.8f,-sp},{-sp,0.8f,-sp},
        { sp,0.8f,  0},{-sp,0.8f,  0},{ 0,0.8f, sp},{ 0,0.8f,-sp},
        { sp,0.8f, 10},{ sp,0.8f,-10},{-sp,0.8f, 10},{-sp,0.8f,-10},
        { 10.0f,0.8f, sp},{-10.0f,0.8f, sp},{ 10.0f,0.8f,-sp},{-10.0f,0.8f,-sp}
    };
}
...
const std::vector<DirectX::SimpleMath::Vector4> GameSceneWorldArena::GetWallRects() const
{
    std::vector<DirectX::SimpleMath::Vector4> rects;
    rects.reserve(m_walls.size());
    for (const WallData& w : m_walls)
    {
        rects.emplace_back(w.minX, w.minZ, w.maxX, w.maxZ);
    }
    return rects;
}
