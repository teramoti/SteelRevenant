...
namespace
{
    ...
    std::wstring ResolveAudioRoot()
    {
        ...
        {
            fs::path(L"Assets") / L"Audio",
            fs::path(L"..") / L"Assets" / L"Audio",
            fs::path(L"..") / L".." / L"Assets" / L"Audio",
            fs::path(L"..") / L".." / L".." / L"Assets" / L"Audio"
        };
        ...
        return (fs::current_path() / L"Assets" / L"Audio").wstring();
    }

    std::unique_ptr<DirectX::SpriteFont> LoadGameFont(ID3D11Device* device)
    {
        static const std::array<const wchar_t*, 12> kFontCandidates =
        {
        ...
        for (const wchar_t* candidate : kFontCandidates)
        {
            try
            {
                auto font = std::make_unique<DirectX::SpriteFont>(device, candidate);
...
void Game::Initialize()
{
    ...
    m_font        = LoadGameFont(m_directX.GetDevice().Get());
...
// 現在フレームを描画する。
// 3D描画(GeometricPrimitive等)とSpriteBatch 2D描画の順序は各シーンが制御する。
void Game::Render()
{
