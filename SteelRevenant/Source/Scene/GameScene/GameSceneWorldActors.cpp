//------------------------//------------------------
// Contents(処理内容) プレイヤー・敵・ロックオンリングの描画を実装する。
// Bug#2修正: DirectX 左手座標系では RotationX の正値が後方へ傾く。
//           中段構えには bladePitch = -0.55f (負値) が正しい。
//           修正前: +0.55f → 剣が後方を向く
//           修正後: -0.55f → 剣が前方(正面)を向く
//------------------------//------------------------
    ...
    const Matrix& bodyWorld, const Action::PlayerState& player)
{
    ...
    // Bug#2修正: 中段構え = bladePitch -0.55f (左手座標系で前方向き)
