...
void GameScene::DrawWorldActors()
{
	...
	{
		Vector3(0.16f, 0.98f + std::fabs(runCycle) * 0.010f, 0.04f),
		-0.03f,
		std::fabs(runCycle) * 0.012f,
		0.00f,
		0.18f,
		-0.34f,
		0.04f
	...
	const Vector3 frontGripWorld = rearGripWorld + bladeDir * 0.16f;
	const Vector3 guardCenter = rearGripWorld + bladeDir * 0.24f;
	const Vector3 weaponCenter = rearGripWorld + bladeDir * 0.96f;
	const Vector3 leftShoulderWorld = Vector3::Transform(Vector3(-0.22f, 1.18f, 0.03f), pRoot);
	const Vector3 rightShoulderWorld = Vector3::Transform(Vector3(0.22f, 1.18f, 0.03f), pRoot);
