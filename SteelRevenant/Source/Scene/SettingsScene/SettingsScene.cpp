...
namespace
{
    ...
    // one extra control for "Reset to Defaults"
    constexpr int kSettingControlCount = 5;
    constexpr int kResetItemIndex = kSettingControlCount;
    constexpr int kBackItemIndex = kSettingControlCount + 1;
    constexpr int kTitleItemIndex = kSettingControlCount + 2;
    constexpr int kQuitItemIndex = kSettingControlCount + 3;
    constexpr int kMenuItemCount = kSettingControlCount + 4;
...
void SettingsScene::Update(const DX::StepTimer& stepTimer)
{
    ...
    const Vector2 resetPos(panelPos.x + 40.0f * scale, panelPos.y + panelSize.y - 228.0f * scale);
    ...
    if (UiUtil::IsInsideRect(mousePoint, resetPos, buttonSize))
    {
        m_selectedItem = kResetItemIndex;
    }
    ...
    if (mouseTracker->leftButton == DirectX::Mouse::ButtonStateTracker::PRESSED)
    {
        ...
        if (UiUtil::IsInsideRect(mousePoint, resetPos, buttonSize))
        {
            ...
            const float defaultMouse = 0.12f;
            const GameAudio::AudioVolumeSettings defaultVol{}; // defaults to 1.0f
            ApplyMouseSensitivity(defaultMouse);
            GameAudio::AudioSystem::GetInstance().ApplyVolumeSettings(defaultVol);
            GameSaveData::GetInstance().SetMouseSensitivity(defaultMouse);
            GameSaveData::GetInstance().SetAudioVolumeSettings(defaultVol);
            SceneUiSound::PlayConfirm();
            return;
        }
    ...
    if (keyboardTracker->pressed.Enter || keyboardTracker->pressed.Space)
    {
        ...
        if (m_selectedItem == kResetItemIndex)
        {
            // reset to defaults via keyboard
            SceneUiSound::PlayConfirm();
            const float defaultMouse = 0.12f;
            const GameAudio::AudioVolumeSettings defaultVol{};
            ApplyMouseSensitivity(defaultMouse);
            GameAudio::AudioSystem::GetInstance().ApplyVolumeSettings(defaultVol);
            GameSaveData::GetInstance().SetMouseSensitivity(defaultMouse);
            GameSaveData::GetInstance().SetAudioVolumeSettings(defaultVol);
...
void SettingsScene::Render()
{
    ...
    drawButton(resetPos, L"デフォルトに戻す", L"感度と音量をデフォルト値に戻します", m_selectedItem == kResetItemIndex, Color(0.12f, 0.18f, 0.24f, 0.92f));
